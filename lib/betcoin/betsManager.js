...
class BetsManager {
  ...
  getBetsByEvent (event) {
    return R.filter(R.propEq('data.bet.betEvent', eventId))(this.getAllBets())
  }

  getBetsByEventAndBetType (event, type) {
    return R.filter(R.anyPass(R.propEq('data.bet.betEvent', eventId),))(this.getAllBets())
  }
