...
class Blockchain {
  ...
  rename (name) {
    if (this.dbName !== name) {
      this.dbName = name
      this.blocksDb.rename('data/' + this.dbName + '/' + BLOCKCHAIN_FILE)
      this.transactionsDb.rename('data/' + this.dbName + '/' + TRANSACTIONS_FILE)
      this.blocks       = this.blocksDb.read(Blocks)
      this.transactions = this.transactionsDb.read(Transactions)
    }
  }
  ...
  init () {
    ...
    if (this.blocks.length == 0) {
      this.logger.info('Blockchain empty, adding genesis block')
      this.blocks.push(Block.genesis)
      this.blocksDb.write(this.blocks)
    }

    // Remove transactions that are in the blockchain
    this.logger.info('Removing transactions that are in the blockchain')
  ...
  getDifficulty (index) {
    // INFO: The difficulty is the formula that betcoin choose to check the proof a work, this number is later converted to base 16 to represent the minimal initial hash expected value
  ...
  getTransactionFromBlocksById (transactionId) {
    let block = R.find(R.compose(R.find(R.propEq('id', transactionId)), R.prop('transactions')), this.blocks)

    if (block) {
      return block.transactions.pop()
    }

    return null
  }

  getTransactionFromBlocksByHash (transactionHash) {
    let block = R.find(R.compose(R.find(R.propEq('hash', transactionHash)), R.prop('transactions')), this.blocks)
  ...
  getAllTransactionFromBlocks () {
    return R.flatten(R.map(R.prop('transactions'), this.blocks))
  }

  replaceChain (newBlockchain) {
    // It doesn't make sense to replace this blockchain by a smaller one
    if (newBlockchain.length <= this.blocks.length) {
      this.logger.error('Blockchain shorter than the current blockchain')
  ...
  checkChain (blockchainToValidate) {
    ...
    if (JSON.stringify(blockchainToValidate[0]) !== JSON.stringify(Block.genesis)) {
      this.logger.error('Genesis blocks aren\'t the same')
    ...
    try {
      ...
      this.logger.error('Invalid block sequence')
  ...
  addBlock (newBlock, emit = true) {
    ...
    if (this.checkBlock(newBlock, this.getLastBlock())) {
      ...
      this.logger.info(`Block added: ${newBlock.hash}`)
      this.logger.debug(`Block added: ${JSON.stringify(newBlock)}`)
  ...
  addTransaction (newTransaction, emit = true) {
    ...
    if (this.checkTransaction(newTransaction)) {
      ...
      this.logger.info(`Transaction added: ${newTransaction.id}`)
      this.logger.debug(`Transaction added: ${JSON.stringify(newTransaction)}`)
  ...
  checkBlock (newBlock, previousBlock) {
    ...
    if (previousBlock.index + 1 !== newBlock.index) { // Check if the block is the last one
      this.logger.error(`Invalid index: expected '${previousBlock.index + 1}' got '${newBlock.index}'`)
      ...
      this.logger.error(`Invalid previoushash: expected '${previousBlock.hash}' got '${newBlock.previousHash}'`)
      ...
      this.logger.error(`Invalid hash: expected '${blockHash}' got '${newBlock.hash}'`)
      ...
      this.logger.error(`Invalid proof-of-work difficulty: expected '${newBlock.getDifficulty()}' to be smaller than '${this.getDifficulty(newBlock.index)}'`)
    ...
    let regularTransactions = R.filter(R.anyPass([R.propEq('type', 'regular'), R.propEq('type', 'regular'), R.propEq('type', 'regular')]))(newBlock.transactions)
    ...
    if (regularTransactions.length) {
      ...
      let sumOfInputsAmount  = R.sum(R.flatten(R.map(R.compose(R.map(R.prop('amount')), R.prop('inputs'), R.prop('data')), regularTransactions))) + MINING_REWARD
      let sumOfOutputsAmount = R.sum(R.flatten(R.map(R.compose(R.map(R.prop('amount')), R.prop('outputs'), R.prop('data')), regularTransactions)))
      ...
      if (!isInputsAmountGreaterOrEqualThanOutputsAmount) {
        this.logger.error(`Invalid block balance: inputs sum '${sumOfInputsAmount}', outputs sum '${sumOfOutputsAmount}'`)
    ...
    if (transactionsByType.fee && transactionsByType.fee > 1) {
      this.logger.error(`Invalid fee transaction count: expected '1' got '${transactionsByType.fee}'`)
    ...
    if (transactionsByType.reward && transactionsByType.reward > 1) {
      this.logger.error(`Invalid reward transaction count: expected '1' got '${transactionsByType.reward}'`)
  ...
  checkTransaction (transaction) {
    ...
    if (!isNotInBlockchain) {
      this.logger.error(`Transaction '${transaction.id}' is already in the blockchain`)
    ...
    if (transaction.type == 'regular') {
      ...
      if (!isInputTransactionsUnspent) {
        this.logger.error(`Not all inputs are unspent for transaction '${transaction.id}'`)
  ...
  getUnspentTransactionsForAddress (address) {
    ...
    let txOutputs = []
    R.forEach(R.pipe(R.prop('transactions'), R.filter(R.anyPass([R.propEq('type', 'regular'), R.propEq('type', 'fee'), R.propEq('type', 'bet'), R.propEq('type', 'gain'), R.propEq('type', 'reward')])), R.forEach((transaction) => {
    ...
    let txInputs = []
    R.forEach(R.pipe(R.prop('transactions'), R.filter(R.anyPass([R.propEq('type', 'regular'), R.propEq('type', 'bet'), R.propEq('type', 'gain')])), R.forEach(
