...
class Blockchain {
  ...
  init () {
    ...
    R.forEach(this.removeBlockTransactionsFromPendingTransactions.bind(this), this.blocks)
  }
  ...
  addBlock (newBlock, emit = true) {
    ...
    if (this.checkBlock(newBlock, this.getLastBlock())) {
      ...
      this.removeBlockTransactionsFromPendingTransactions(newBlock)

      this.logger.info(`Block added: ${newBlock.hash}`)
      this.logger.debug(`Block added: ${JSON.stringify(newBlock, null, 2)}`)
      if (emit) {
  ...
  removeBlockTransactionsFromPendingTransactions (newBlock) {
