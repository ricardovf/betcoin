  ...
  check () {
    ...
    if (this.type == 'regular') {
      ...
      if (!isInputsAmountGreaterOrEqualThanOutputsAmount) {
        console.error(`Invalid transaction balanceL: inputs sum '${sumOfInputsAmount}', outputs sum '${sumOfOutputsAmount}'`)
      ...
      if (!isEnoughFee) {
        ...
        throw new TransactionAssertionError(`Not enough fee: expected '${FEE_PER_TRANSACTION}' got '${(sumOfInputsAmount - sumOfOutputsAmount)}'`, {
          sumOfInputsAmount,
          sumOfOutputsAmount,
          FEE_PER_TRANSACTION
        })
