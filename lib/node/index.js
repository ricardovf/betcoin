...
class Node {
  constructor (host, port, peers, blockchain, logger) {
    this.host       = host
    this.port       = port
    this.peers      = []
  ...
  connectToPeers (newPeers) {
    ...
    newPeers.forEach((peer) => {
      ...
      if (!this.peers.find((element) => { return element.url == peer.url }) && peer.url != me) {
        ...
        this.logger.info(`Peer ${peer.url} added to connections.`)
        this.peers.push(peer)
        this.initConnection(peer)
        this.broadcast(this.sendPeer, peer)
      } else {
        this.logger.info(`Peer ${peer.url} not added to connections, because I already have.`)
      }
    }, this)
  }
  ...
  sendPeer (peer, peerToSend) {
    ...
    this.logger.info(`Sending ${peerToSend.url} to peer ${URL}.`)
    return superagent
      .post(URL)
      .send(peerToSend)
      .catch((err) => {
        this.logger.warn(`Unable to send me to peer ${URL}: ${err.message}`)
      })
  }
  ...
  getLatestBlock (peer) {
    ...
    let self  = this
    this.logger.info(`Getting latest block from: ${URL}`)
    return superagent
      .get(URL)
      .then((res) => {
      ...
      .catch((err) => {
        this.logger.warn(`Unable to get latest block from ${URL}: ${err.message}`)
      })
  }

  sendLatestBlock (peer, block) {
    const URL = `${peer.url}/blockchain/blocks/latest`
    this.logger.info(`Posting latest block to: ${URL}`)
    return superagent
      .put(URL)
      .send(block)
      .catch((err) => {
        this.logger.warn(`Unable to post latest block to ${URL}: ${err.message}`)
      })
  }
  ...
  getBlocks (peer) {
    ...
    let self  = this
    this.logger.info(`Getting blocks from: ${URL}`)
    return superagent
      .get(URL)
      .then((res) => {
      ...
      .catch((err) => {
        this.logger.warn(`Unable to get blocks from ${URL}: ${err.message}`)
      })
  }

  sendTransaction (peer, transaction) {
    const URL = `${peer.url}/blockchain/transactions`
    this.logger.info(`Sending transaction '${transaction.id}' to: '${URL}'`)
    return superagent
      .post(URL)
      .send(transaction)
      .catch((err) => {
        this.logger.warn(`Unable to put transaction to ${URL}: ${err.message}`)
      })
  }
  ...
  getTransactions (peer) {
    const URL = `${peer.url}/blockchain/transactions`
    let self  = this
    this.logger.info(`Getting transactions from: ${URL}`)
    return superagent
      .get(URL)
      .then((res) => {
      ...
      .catch((err) => {
        this.logger.warn(`Unable to get transations from ${URL}: ${err.message}`)
      })
  }
  ...
  getConfirmations (transactionId) {
    ...
    let foundLocally = this.blockchain.getTransactionFromBlocksById(transactionId) !== null
  ...
  syncTransactions (transactions) {
    ...
    R.forEach((transaction) => {
      let transactionFound = this.blockchain.getPendingTransactionById(transaction.id)

      if (transactionFound == null) {
        this.logger.info(`Syncing transaction '${transaction.id}'`)
  ...
  checkReceivedBlocks (blocks) {
    ...
    if (latestBlockReceived.index <= latestBlockHeld.index) {
      this.logger.info('Received blockchain is not longer than blockchain. Do nothing')
      return false
    }

    this.logger.info(`Blockchain possibly behind. We got: ${latestBlockHeld.index}, Peer got: ${latestBlockReceived.index}`)
    if (latestBlockHeld.hash === latestBlockReceived.previousHash) { // We can append the received block to our chain
      this.logger.info('Appending received block to our chain')
      this.blockchain.addBlock(latestBlockReceived)
      return true
    } else if (receivedBlocks.length === 1) { // We have to query the chain from our peer
      this.logger.info('Querying chain from our peers')
      this.broadcast(this.getBlocks)
      return null
    } else { // Received blockchain is longer than current blockchain
      this.logger.info('Received blockchain is longer than current blockchain')
