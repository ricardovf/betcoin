...
const TRANSACTIONS_TO_PROCESS = 10
...
class Miner {
  ...
  mine (address) {
    let baseBlock = Miner.generateNextBlock(address, this.blockchain.getLastBlock(), this.blockchain.transactions, this.resultsManager)
  ...
  static generateNextBlock (address, previousBlock, blockchainTransactions, resultsManager) {
    ...
    R.forEach((result) => {
      transactions.push(resultsManager.calculateGainsTransaction(result))
    }, resultTransactions)
