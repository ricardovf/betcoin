...
class HttpServer {
  constructor (node, blockchain, operator, eventsManager, betsManager, resultsManager, miner, logger) {
    ...
    this.app.get('/blockchain/processed-transactions', (req, res) => {
      res.status(200).send(blockchain.getAllTransactionFromBlocks())
    })

    this.app.get('/blockchain/events', (req, res) => {
      res.status(200).send(eventsManager.getAllEvents())
    })

    this.app.get('/blockchain/results', (req, res) => {
      res.status(200).send(resultsManager.getAllResults())
    })


    this.app.get('/blockchain/bets', (req, res) => {
      res.status(200).send(betsManager.getAllBets())
    })
    ...
    this.app.post('/events', (req, res) => {
      try {
        let newTransaction = eventsManager.createEvent(req.body.eventType, req.body.betType, req.body.date, req.body.teams)
        let transactionCreated = blockchain.addTransaction(Transaction.fromJson(newTransaction))
        res.status(201).send(transactionCreated)
      } catch (ex) {
        if (ex instanceof ArgumentError || ex instanceof TransactionAssertionError) throw new HTTPError(400, ex.message, walletId, ex)
    ...
    this.app.post('/operator/wallets/:walletId/bets', (req, res) => {
      ...
      try {
        ...
        let newTransaction = betsManager.createBet(req.body.betEvent, req.body.betType, req.body.betOn, walletId, req.body.fromAddressId, req.body.amount)
    ...
    this.app.post('/results', (req, res) => {
      try {
        let newTransaction = resultsManager.createResult(req.body.resultEvent, req.body.resultBetType, req.body.result)
        let transactionCreated = blockchain.addTransaction(Transaction.fromJson(newTransaction))
        res.status(201).send(transactionCreated)
      } catch (ex) {
        if (ex instanceof ArgumentError || ex instanceof TransactionAssertionError) throw new HTTPError(400, ex.message, walletId, ex)
