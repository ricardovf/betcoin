...
class HttpServer {
  constructor (node, blockchain, operator, miner, logger) {
    ...
    this.app.get('/blockchain/blocks/latest', (req, res) => {
      ...
      if ( ! lastBlock) throw new HTTPError(404, 'Last block not found')
    ...
    this.app.put('/blockchain/blocks/latest', (req, res) => {
      ...
      if ( ! result) res.status(200).send('Requesting the blockchain to check.')
    ...
    this.app.get('/blockchain/blocks/:hash([a-zA-Z0-9]{64})', (req, res) => {
      ...
      if ( ! blockFound) throw new HTTPError(404, `Block not found with hash '${req.params.hash}'`)
    ...
    this.app.get('/blockchain/blocks/:index', (req, res) => {
      ...
      if ( ! blockFound) throw new HTTPError(404, `Block not found with index '${req.params.index}'`)

      res.status(200).send(blockFound)
    })

    this.app.get('/blockchain/blocks/transactions/:transactionId([a-zA-Z0-9]{64})', (req, res) => {
      let transactionFromBlock = blockchain.getTransactionFromBlocksById(req.params.transactionId)
      if ( ! transactionFromBlock) throw new HTTPError(404, `Transaction '${req.params.transactionId}' not found in any block`)

      res.status(200).send(transactionFromBlock)
    })

    this.app.get('/blockchain/transactions', (req, res) => {
      res.status(200).send(blockchain.getAllPendingTransactions())
    })

    this.app.post('/blockchain/transactions', (req, res) => {
      let requestTransaction = Transaction.fromJson(req.body)
      let transactionFound = blockchain.getPendingTransactionById(requestTransaction.id)

      if ( ! transactionFound) throw new HTTPError(409, `Transaction '${requestTransaction.id}' already exists`)
    ...
    this.app.get('/operator/wallets/:walletId', (req, res) => {
      ...
      if ( ! walletFound) throw new HTTPError(404, `Wallet not found with id '${req.params.walletId}'`)
    ...
    this.app.post('/operator/wallets/:walletId/transactions', (req, res) => {
      ...
      if ( ! password) throw new HTTPError(401, 'Wallet\'s password is missing.')
  ...
  listen (host, port) {
    return new Promise((resolve, reject) => {
      this.server = this.app.listen(port, host, (err) => {
        if (err) reject(err)
        this.logger.info(`Listening http on port: ${this.server.address().port}, to access the API documentation go to http://${host}:${this.server.address().port}/api-docs/`)
        resolve(this)
      })
    })
  }
